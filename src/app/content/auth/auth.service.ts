...
export class AuthService {
  ...
  getCurrentUser(): Observable<CurrentUserInterface> {
    return this.http.get<AuthResponseInterface>('api/user').pipe(map((r) => this.getUser(r)));
  }

  register(data: RegisterRequest): Observable<CurrentUserInterface> {
    return this.http.post<AuthResponseInterface>('api/users', data).pipe(map((r) => this.getUser(r)));
  }

  login(data: LoginRequest): Observable<CurrentUserInterface> {
    return this.http.post<AuthResponseInterface>('api/users/login', data).pipe(map((r) => this.getUser(r)));
  }
